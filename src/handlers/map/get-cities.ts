...
export default class MapGetCities extends OpenMethodAPI<IFieldsGetCitiesParams, IApiList<ICity>> {
    protected handleParams(params: IApiParams, props: ICallPropsOpen): IFieldsGetCitiesParams {
        const area = parseAndCheckArea(params.area as string);

        return { area };
    }

    protected async perform(params: IFieldsGetCitiesParams, { database, session }: ICallPropsOpen): Promise<IApiList<ICity>> {
