...
## Collection fields
В некоторых методах возможно передать параметр `fields` для получения опциональной дополнительной информации о коллекции. Чтобы получить информацию, нужно перечислить через запятую ключи. Доступны следующие ключи:
* `collection_city` - информация о городе, который прикреплён к коллекции;
* `collection_rating` - информация об оценках коллекции.
